...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	private void generateRevealAnimate(final View view, final boolean swap,
		...
		// 有时报animate空指针，需继续研究下代码
		try {
			// backView滑动、开关动画和位移
			if (swipingRight) {// 向右滑
				if (!opened.get(position)) {// item是非打开状态
					animate(backView).translationX(moveTo - viewWidth)
							.setDuration(animationTime);// 左边backView的动画效果
				} else {// item打开状态
					animate(backView).translationX(viewWidth + moveTo)
							.setDuration(animationTime);
				}
			} else {// 向左滑
				if (!opened.get(position)) {// item是非打开状态
					animate(backView).translationX(viewWidth + moveTo)
							.setDuration(animationTime);// 右边backView的动画效果
				} else {// item打开状态
					animate(backView).translationX(moveTo - viewWidth)
							.setDuration(animationTime);
				}
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity
				...
				// 如果是向右滑动，松开手指时，距离为小于定义显示控件的距离则close，大于定义显示控件的距离则open；反之相同
			} else if (swipingRight ? Math.abs(deltaX) > ((viewWidth - rightOffset) / 2)
					: Math.abs(deltaX) > ((viewWidth - leftOffset) / 2)) {
				swap = true;
				swapRight = deltaX > 0;
			}
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (swiping && downPosition != ListView.INVALID_POSITION) {
				...
				if (swipeMode == SwipeListView.SWIPE_MODE_BOTH) {
					move(deltaX);
				} else if (swipeMode == SwipeListView.SWIPE_MODE_LEFT) {
					if (deltaX < 0) {
						move(deltaX);
					}
				} else if (swipeMode == SwipeListView.SWIPE_MODE_RIGHT) {
	...
	public void move(float deltaX) {
		...
		if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
			...
			setTranslationX(frontView, deltaX);
			if (swipingRight) {
				// backView跟随frontView往右滑动
				if (opened.get(downPosition)) {
					if ((viewWidth + deltaX) > leftOffset) {
						setTranslationX(backView, deltaX + viewWidth);
					}
				} else {
				...
				// backView跟随frontView往左滑动
				if (opened.get(downPosition)) {// 如果是打开状态
					if (deltaX <= (viewWidth - rightOffset)) {
						setTranslationX(backView, deltaX - viewWidth);
					}
				} else {
